...
核心理念
每一个 emoji 相当于一个"像素格"，通过在网格上排列不同颜色的 emoji 来构建图像。 每行 emoji 数量要保持一致（用空白 emoji 　 或透明占位符补齐），确保网格整齐。
...
设计流程
草图规划：先在脑中（或文字描述中）把图像分解为 N×M 的像素网格
配色选择：确定主色、阴影色、高光色各用哪个 emoji
逐行绘制：从上到下，逐行填写 emoji，每行长度相同
细节优化：检查轮廓是否流畅，高光/阴影是否自然
像素熊示例（参考风格：方块像素游戏角色）
　🟫🟫　　🟫🟫　
🟫🟫🟫🟫🟫🟫🟫🟫
🟫🟫⬛🟫🟫⬛🟫🟫
